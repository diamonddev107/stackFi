...
async function addMarkets(diamondAddress) {
  ...
  console.log(
    "3000000000 tusdt transfered to faucet. Token being :",
    tUsdtAddress
  );
  ...
  console.log(
    "3000000000 tusdc transfered to faucet. Token being :",
    tUsdcAddress
  );
  ...
  console.log(
    "1200000 twbnb transfered to faucet. Token being :",
    tWBNBAddress
  );
  console.log(await twbnb.balanceOf(faucet.address));

  await faucet._updateTokens(tUsdtAddress, "10000000000000000000000"); // 10000 USDT
  await faucet._updateTokens(tUsdcAddress, "10000000000000000000000"); // 10000 USDC
  await faucet._updateTokens(tBtcAddress, "500000000"); // 5 BTC
  await faucet._updateTokens(tWBNBAddress, "100000000000000000000"); // 100 BNB

  addrAccount1 = await accounts[1].getAddress();
  console.log("Address acount1:", addrAccount1);
  console.log("Account1 USDT bal :", await tusdt.balanceOf(addrAccount1));
  await faucet.connect(accounts[1]).getTokens(addrAccount1, 0);
  console.log(
    "Transfer funds to account1",
    await tusdt.balanceOf(addrAccount1)
  );
  console.log("USDT balance of faucet", await tusdt.balanceOf(faucet.address));
