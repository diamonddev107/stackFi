...
contract Loan1 is Pausable, ILoan1 {
    ...
    function hasLoanAccount(address _account)
        external
        view
        override
        returns (bool)
    ...
    function avblReservesLoan(bytes32 _market)
        external
        view
        override
        returns (uint256)
    ...
    function utilisedReservesLoan(bytes32 _market)
        external
        view
        override
        returns (uint256)
    ...
    function liquidation(address _account, uint256 _id)
        external
        override
        nonReentrant
        authLoan1
    ...
    function permissibleWithdrawal(
        bytes32 _market,
        bytes32 _commitment,
        bytes32 _collateralMarket,
        uint256 _amount
        ...
        return
            LibOpen._permissibleWithdrawal(
                _market,
                _commitment,
                _collateralMarket,
                _amount,
...
<<<<<<< HEAD
        ...
        require(
            LibOpen._hasAdminRole(ds.superAdmin, ds.superAdminAddress) ||
                LibOpen._hasAdminRole(ds.adminLoan1, ds.adminLoan1Address),
            "ERROR: Not an admin"
        );
=======
    	AppStorageOpen storage ds = LibOpen.diamondStorage(); 
